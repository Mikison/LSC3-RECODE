...
public class addHealth extends subcommand {

    private LSC3 plugin;

    public addHealth(LSC3 plugin) {
        this.plugin = plugin;
    }
    ...
    public Component getSyntax() {
        return util.mm("<click:suggest_command:/lifesteal add><hover:show_text:'<aqua>/lifesteal add [player] [amount] <-s><newline><newline><gray>Adding specified amount of health to player'>  <red>• <gray>/lifesteal add [player] [amount] <-s></hover></click>");
    }
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        if (args[1].equals("*")) {
            ...
            sender.sendMessage(MiniMessage.miniMessage().deserialize(plugin.messagess.getMessageConfig().getString("messagess.COMMAND_ADD_SUCCESS_SENDER_SILENT_ALL"), Placeholder.parsed("amount", String.valueOf(args[2])), Placeholder.parsed("count" , String.valueOf(count)), Placeholder.parsed("prefix", plugin.messagess.getMessageConfig().getString("messagess.prefix"))));
            if (sender instanceof Player player) player.playSound(player, Sound.BLOCK_NOTE_BLOCK_BELL, 0.3F, 1);
            return;
        }
        Player target = Bukkit.getPlayer(args[1]);
        if (!commandMethod(sender, target, args)) return;
        sender.sendMessage(MiniMessage.miniMessage().deserialize(plugin.messagess.getMessageConfig().getString("messagess.COMMAND_ADD_SUCCESS_SENDER_SILENT"), Placeholder.parsed("amount", String.valueOf(args[2])), Placeholder.parsed("target", target.getName()), Placeholder.parsed("prefix", plugin.messagess.getMessageConfig().getString("messagess.prefix"))));
        if (sender instanceof Player player) player.playSound(player, Sound.BLOCK_NOTE_BLOCK_BELL, 0.3F, 1);
    }
    ...
    private boolean commandMethod(CommandSender sender,Player target, String[] args) {
        ...
        if (PlayerHealth / 2 + Double.parseDouble(args[2]) > dataConfig.getDataCfg().getInt("profiles." + target.getUniqueId() + ".maxHealth")) {
            sender.sendMessage(MiniMessage.miniMessage().deserialize(plugin.messagess.getMessageConfig().getString("messagess.COMMAND_ADD_EXCEEDING_VALUE"), Placeholder.parsed("target", target.getName()), Placeholder.parsed("amount", String.valueOf(PlayerHealth / 2)), Placeholder.parsed("max_amount", String.valueOf(dataConfig.getDataCfg().getInt("profiles." + target.getUniqueId() + ".maxHealth"))), Placeholder.parsed("prefix", plugin.messagess.getMessageConfig().getString("messagess.prefix"))));
            return false;
        }
